...
| 3 | Compilación de assets | ✅ ya resuelto, sin ajuste necesario | El skill `git-workflow` ya documentaba correctamente que `npm run build` y `composer install --no-dev` corren **en local**, y los artefactos compilados (`vendor/`, `public/build/`, `public/css|js/filament`) se comitean a la rama `deploy` con `git add -f`. cPanel solo hace `Git Version Control → Update from Remote`. No compila nada — no hay riesgo de que falte Node.js en el servidor. **Hito 3b-3, segunda vuelta:** el panel `/academia` tiene su propio tema Vite (`resources/css/filament/academic/theme.css`, registrado en `AcademicPanelProvider::viteTheme()`) — sin esto, Filament servía su CSS por defecto del paquete, que no incluye ninguna clase de Tailwind propia del proyecto (confirmado en vivo: 0 matches para clases como `rounded`/`bg-amber-100` en ese bundle). Es una entrada más del mismo `input` de `vite.config.js` que ya compila `resources/css/app.css` — el mismo `npm run build` de este mismo paso la genera (`public/build/assets/theme-*.css`), nada nuevo que compilar en el servidor. |
